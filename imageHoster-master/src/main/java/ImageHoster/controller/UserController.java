...
public class UserController {
    ...
    public String registerUser(User user, Model model) {
        ...
        //Code to add password constraints for strong password.
        String passwordConstraint = "^(?=.*[a-zA-Z])(?=.*?\\d)" +
                "(?=.*?[`~!@#$%^&*()\\-_=+\\\\|\\[{\\]};:'\",<.>/?]).*$";

        //pattern matcher iplemented here.
        if(!Pattern.compile(passwordConstraint).matcher(user.getPassword()).matches()) {
